...
def process_row(row):
    # if row['mean_peak_frequency'] == 128:
...
for file_path in eeg_file_paths:
    ...
    # reading seizure labels from annotation CSV
    annotation_file_path = f"{file_path.replace('.csv', '_anno.csv')}"
    df_anno = pd.read_csv(annotation_file_path, header=None, names=['Seizure'])
    
    # initializing the seizure label to 0
    df_channel['seizure_label'] = df_anno['Seizure']
    
    # creating the window size and making it modular in case I want to change it
    window_size = 60 * 32
    ...
    feature_list = []
    for start in range(0, len(df_channel), step_size):
        end = start + window_size
        
        if end > len(df_channel):
            end = len(df_channel)

        window_df = df_channel.iloc[start:end, :]
        print(f"Processing window from {start} to {end} in file {file_path}")

        window_features = {'start': start, 'end': end, 'seizure_label': df_channel['seizure_label'].iloc[start]}
        
        # looping through channels and extracting features
        for channel in df_channel.columns[:-1]:  # excluding the last column (seizure_label)
