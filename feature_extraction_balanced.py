...
for file_path in eeg_file_paths:
    ...
    if seizure_count > 0:
        min_samples = min(seizure_count, non_seizure_count)
        balanced_feature_list = []
        seizure_count = 0
        non_seizure_count = 0
        
        for window_features in feature_list:
            if window_features['seizure_label'] == 1 and seizure_count < min_samples:
                balanced_feature_list.append(window_features)
                seizure_count += 1
            elif window_features['seizure_label'] == 0 and non_seizure_count < min_samples:
                balanced_feature_list.append(window_features)
                non_seizure_count += 1

        balanced_windowed_feature_df = pd.DataFrame(balanced_feature_list)
    ...
    for feature_name in feature_names:
        channel_features = [col for col in balanced_windowed_feature_df.columns if f'{feature_name}' in col]
        balanced_windowed_feature_df[f'mean_{feature_name}'] = balanced_windowed_feature_df[channel_features].mean(axis=1)
        balanced_windowed_feature_df[f'min_{feature_name}'] = balanced_windowed_feature_df[channel_features].min(axis=1)
        balanced_windowed_feature_df[f'max_{feature_name}'] = balanced_windowed_feature_df[channel_features].max(axis=1)

    # Create a copy of the DataFrame before changes
    original_balanced_windowed_feature_df = balanced_windowed_feature_df.copy()

    # Save the original features to a CSV file
    output_file_path = f"CSVFeatures/{file_path.split('/')[-1].replace('.csv', '_features.csv')}"
    original_balanced_windowed_feature_df.to_csv(output_file_path, index=False)
    print(f"Original features saved to {output_file_path}")

    # Apply the process_row function to each row
    balanced_windowed_feature_df = balanced_windowed_feature_df.apply(process_row, axis=1)
    ...
    balanced_windowed_feature_df.to_csv(output_changed_file_path, index=False)
